...
class _StudentListScreenState extends State<TeachersList> {
  ...
  Future<void> postData() async {
    ...
    // API endpoint
    String apiUrl =
        'http://170.249.216.178/~filterba/schooling/public/api/teachers';
    ...
    try {
      ...
      final response = await http.post(
        ...
        body: jsonEncode({'schoolId': schoolId}),
      );
  ...
  Future<void> deleteStudents(String id) async {
    ...
    try {
      var url = Uri.parse('http://170.249.216.178/~filterba/schooling/public/api/teachers-delete');
      var headers = {'Content-Type': 'application/json'};
      var body = jsonEncode({'teacherId': id});
      ...
      builder: (BuildContext context) {
        return SimpleDialog(
          ...
          children: [
                ...
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (context) => UpdateTeacher(
                      teacherId: studentData["id"].toString(), // Assuming 'id' is the student ID
                      name: studentName,
            ...
            InkWell(
              onTap: () {
                deleteStudents(apiResponse["id"].toString());
              },
              child: Container(
                padding: EdgeInsets.all(10),
                margin: EdgeInsets.all(10),
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(
                      ...
                      shape: RoundedRectangleBorder(
                        borderRadius: BorderRadius.circular(20), // Set rounded corner radius
                      ),
                    ),
                    child: Padding(
                      padding:  EdgeInsets.all(10),
                      child: Text('Add Teacher',style: TextStyle(color: Colors.white,fontSize: 16),),
                    ),
                  ),
                ),
              ),
  ...
  Widget _buildStudentListItem(var apiData,var value) {
    return  InkWell(
      onTap: (){
        _showSimpleDialog(context, "1",apiData);
      },
      child: Container(
        padding: EdgeInsets.all(10),
        decoration: BoxDecoration(
        ...
        child: Row(
          ...
          children: [
            CircleAvatarWithNetworkImage(
              imageUrl: "http://170.249.216.178/~filterba/schooling/public/"+apiData["picture"], // Replace with your image URL
