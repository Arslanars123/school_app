...
class InstituteLoginScreen extends StatefulWidget {
  InstituteLoginScreen({super.key});

  @override
  State<InstituteLoginScreen> createState() => _LoginOneScreenState();
}

class _LoginOneScreenState extends State<InstituteLoginScreen> {
  ...
  Future<void> loginUser(String email, String password, String instituteId) async {
    ...
    // API endpoint
    var apiUrl = "http://170.249.216.178/~filterba/schooling/public/api/login-school";
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await prefs.setString("schoolPicture", responsedata["data"]["picture"]);
        await prefs.setString("schoolName", responsedata["data"]["name"]);
        await prefs.setString("schoolId", responsedata["data"]["schoolId"].toString());
        schoolImage = prefs.getString("schoolPicture");
        var schoolName = prefs.getString("schoolName");
        print(schoolName);
        Fluttertoast.showToast(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Center(child:loader == true? CircularProgressIndicator(color:  Color(0xFFCBAC78),):SizedBox())
          ],
        ),
      ),
    );
